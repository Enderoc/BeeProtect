...
# Для Vercel нужно создать экземпляр Flask внутри функции
def create_app():
    app = Flask(__name__)

    # Общий CSS стиль для обеих страниц (оставляем ваш код без изменений)
    common_style = '''
    <style>
        :root {
    ...
    @app.route('/company')
    def company_page():
        html = company_html.replace('{{ common_style }}', common_style).replace('{{ common_script }}', common_script)
        return render_template_string(html)


    @app.route('/project')
    def project_page():
        html = project_html.replace('{{ common_style }}', common_style).replace('{{ common_script }}', common_script)
        return render_template_string(html)
...
# Handler для Vercel
def handler(request):
    # Создаем WSGI-совместимое окружение из запроса Vercel
    from werkzeug.datastructures import Headers
    from werkzeug.wrappers import Response

    # Конвертируем запрос Vercel в формат, понятный Flask
    environ = {
    ...
    # Обрабатываем запрос через Flask
    with app.request_context(environ):
        ...
        # Конвертируем ответ Flask в формат Vercel
        return {
